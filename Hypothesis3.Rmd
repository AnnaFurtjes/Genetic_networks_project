...
#### 1. Format GWAS with columns expected by software
```{r format_files, eval=F,echo=T}
...
for (i in 1:nrow(all_files)){
	...
	ref_file<-ref_file[,c("BP","CHR","SNP","P","EA","OAFREQ")]
	...
	# determine EAF
	# it states in the regenie documentation: https://rgcgithub.github.io/regenie/options/#output
	# that A1FREQ (I named it OAFREQ in the merging step) indicates the frequency of the alternative/other allele
	# GWAMA expects the frequency of the effect allele -> EAF

	merged_file$EAF<-ifelse(merged_file$A1 != merged_file$EA, merged_file$OAFREQ, 1-merged_file$OAFREQ)
	...
	merged_file<-merged_file[,c("SNPID","CHR","BP","EA","OA","EAF","N","Z","P")]
...
```
Note that same script was used for whole brain, but submitted to the cluster separately because it needed more resources.
...
save(LDSCoutput, file="/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/genetic_networks_project/network_ldsc/all_networks_and_cognitivetraits.RData")
```
...
It seems that Tower is driving most of the associations, which is surprising.
...
```
Heritability estimates from genetic PCs cannot be interpreted because they are not phenotypic PCs.



### Calculate the genetic association between network-specific PC sumstats and a genetic factor *g*
