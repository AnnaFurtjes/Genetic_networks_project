...
## 1. Format GWAS with columns expected by GWAMA software
...
for (i in 1:nrow(all_files)){
	...
	## print file
	setwd("/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/genetic_networks_project/GWAMA_format/")
...
## 2. Create the PC sumstats weighted with loadings
...
setwd("/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/scripts/genetic_networks_project/gwama/")
...
setwd("/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/data/standardised_loadings/")
...
for (i in loadings_all_traits){
	...
	setwd("/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/genetic_networks_project/ldsc/")
	...
	setwd("/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/genetic_networks_project/PC_sumstats/")
...
## 3. Format and munge PC sumstats to perform LDSC
This step is more of a sanity check, and shouldn't exclude any additional rows. 
...
info_file<-fread("/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/regenie/step2/GWAS_22chr_noTBVcontrol_Right_transverse_temporal.txt",header=T,data.table=F)
...
for (i in PC_files){
		...
		setwd("/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/genetic_networks_project/PC_header/")
...
Note that the munging step here is only to get the sumstats in shape. No SNPs are excluded because we only included SNPs in the meta-analysis that survived the munging process in the first place.

# Perform LDSC between PC sumstats and cognitive traits
...
setwd("/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/genetic_networks_project/PC_munged/")

# list files you would like to munge
PC_files<-list.files(pattern=".sumstats.gz")
PC_files<-paste0("/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/genetic_networks_project/PC_munged/",PC_files)
print(PC_files)

# list cognitive traits
setwd("/scratch/users/k1894405/genetic_networks/data/cognititve_GWAS")
...
trait.names<-str_remove(trait.names, pattern="/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/genetic_networks_project/PC_munged/")
...
LDSCoutput<-ldsc(traits=traits,
					ld=ld,wld=wld,
					trait.names = trait.names,
					ldsc.log="/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/genetic_networks_project/network_ldsc/ldsc_all_networks_cognitivetraits.log",
					sample.prev=sample.prev,
					population.prev=population.prev,
					stand=T)
					
save(LDSCoutput, file="/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/genetic_networks_project/network_ldsc/all_networks_cognitive.RData")
```
...
heatmap<-
  ...
  scale_fill_gradient2(low="royalblue2",high="indianred4",mid ="white",
...
## Sanity check: Are all network-specific PC sumstats heritable?
```{r PC_sumstats_h2,message=F,warning=F}
temporarywd<-paste0(workingd,"/data/heritability_volumes/")
setwd(temporarywd)
break
h2<-fread("heritability_networks_14072020.txt",header=T)
kable(h2,caption="Heritability of networks",format="markdown",digits = 3)
```
Heritability estimates from genetic PCs cannot be interpreted because they are not phenotypic PCs, but they give an idea of the signal-to-noise ratio contained in the sumstats.



# Calculate genetic correlation between network-specific PC sumstats and a genetic factor *g*
...
load("all_networks_and_cognitivetraits.RData")
ldscoutput<-LDSCoutput
...
## Summarise the model fit across all networks associated with genetic *g*
...
## Print correlations for each network-specific PC sumstats with genetic *g*
```{r print_corr}
network_assoc<-NULL
...
# round your numbers
genetic_cor<-data.frame(lapply(genetic_cor, function(y) if(is.numeric(y)) round(y, 4) else y))
...
kable(genetic_cor,caption="Genetic correlations between PC sumstats and genetic g",format="markdown",digits = 3)
```

Note that the adjusted *p*-values were corrected using false discovery rate (FDR).


![](C:/Users/k1894405/OneDrive - King's College London/PhD/Projects/Genomic SEM project/Rmarkdown/Path_diagram_central_exec.jpg)
...
# fileparts combined using: cat Brainage_GWAS_sumstat_final.gz* > /scratch/users/k1894405/genetic_networks/data/brain_age_GWAS/Brainage_GWAS_sumstat_final.gz
...
########################
setwd("/scratch/users/k1894405/genetic_networks/data/brain_age_GWAS/")
brainage<-fread("Brainage_GWAS_sumstat_final.txt",header=T,data.table=F)

brainage$N<-45615

fwrite(brainage, file="/scratch/users/k1894405/genetic_networks/data/brain_age_GWAS/brain_age_gwas_header.txt",quote=FALSE,col.names=TRUE,row.names=F,sep=" ",na="NA")
...
traits<-c("/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/genetic_networks_project/brainage/brainage.sumstats.gz","/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/genetic_networks_project/PC_munged/whole_brain.sumstats.gz")
...
LDSCoutput<-ldsc(traits=traits,
					ld=ld,wld=wld,
					trait.names = trait.names,
					ldsc.log="/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/genetic_networks_project/brainage/whole_brain_vs_brainage.log",
					sample.prev=sample.prev,
					population.prev=population.prev,
					stand=T)
					
save(LDSCoutput, file="/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/genetic_networks_project/brainage/whole_brain_vs_brainage.RData")
```
```{r}
