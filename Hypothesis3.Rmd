...
correlation than other networks, when accounting for the number of included nodes.
...
![](C:/Users/k1894405/Documents/GitHub/GenomicPCA project/Genetic_networks_project/methods_visualisation.png)
URLs: [1 GWAMA software package by Baselmans](https://github.com/baselmans/multivariate_GWAMA/), [2 our modified GWAMA function](https://github.com/AnnaFurtjes/Genetic_networks_project/blob/main/my_GWAMA_26032020.R), [3 Step-by-step demonstration of Genomic PCA](https://annafurtjes.github.io/genomicPCA/)
...
library(DT)
datatable(only_networks, caption = "Genetic correlations among genetic networks",colnames = c("Network 1","Network 2","rg"), rownames=FALSE,options = list(pageLength=5, scrollX=T))

```

The genetic correlations between the networks range from `r max(only_networks$value,na.rm=T)` to `r min(only_networks$value,na.rm=T)` (mean rg `r round(mean(only_networks$value,na.rm=T),digits=2)`; SD = `r round(sd(only_networks$value,na.rm=T),digits=2)`). This indicates that the different networks index very similar polygenic signal. The largest genetic correlation of `r max(only_networks$value,na.rm=T)` was between `r only_networks$Var1[which(only_networks$value==max(only_networks$value,na.rm=T))]` and `r only_networks$Var2[which(only_networks$value==max(only_networks$value,na.rm=T))]`. The smallest genetic correlation of `r min(only_networks$value,na.rm=T)` was between `r only_networks$Var1[which(only_networks$value==min(only_networks$value,na.rm=T))]` and `r only_networks$Var2[which(only_networks$value==min(only_networks$value,na.rm=T))]`.
...
```{r calculate_corr, eval=T,warning=F, message=F,results='hide', error=TRUE}
#load dependencies
...
kable(fit_table,caption="Model fit across models",format="markdown",digits = c(0,2,0,24,2,2,2),row.names = F)
```
...
kable(genetic_cor,caption="Genetic correlations between PC sumstats and genetic g",format="markdown", row.names = F)
```
...
![](C:/Users/k1894405/Documents/GitHub/GenomicPCA project/Genetic_networks_project/Path_diagram_central_exec.jpg)
...
### Model fit constrained vs. freely varying models
...
kable(display_fit_other,caption = "Other fit indices of constrained vs. freely varying correlations",digits = 2)
```
...
kable(display_chi,caption="Significance test of difference in chi-squared values obtained as model fit in constrained vs. freely varying correlations",format="markdown",digits = c(0,2,0,2,2))
...
```{r indiv_results, eval=T,warning=F,message=F,results='hide'}
library(stringr)
library(data.table)
#library(devtools)
library(GenomicSEM)
...
results<-ls(pattern="_indivresults")
```
*There are no fit statistics and zero degrees of freedom because the models are fully saturated.*

```{r display_indiv_results}
table<-data.frame()
for (i in results){
    cor_results<-get(i)
...
library(DT)
datatable(table, caption = "Genetic correlations between individual networks and specific cognitive abilities", rownames=FALSE,options = list(pageLength=5, scrollX=T))

```


**Most genetic associations between the networks and individual cognitive traits were significant.**
...
kable(non_sig_models,row.names = F,caption = "Genetic correlations that were non-significant at an FDR adjusted p-value larger than 5%",digits = 2)
```
...
The analyses above, aiming to determine contributions of specific cognitive abilities to the genetic associations between general cognitive ability and networks, are somewhat inclusive. Most traits were significantly associated with the networks. We employ a technique called [Qtrait heterogeneity index](https://www.medrxiv.org/content/10.1101/2020.09.22.20196089v1.full.pdf) to disentangle whether model fit significantly declines, if the model described above allows for direct associations between the networks and specific cognitive abilities. A significant Qtrait statistic would suggest that model fit significantly declined and that the association of interest is likely driven by specific cognitive abilities, rather than going through the general cognitive ability factor.
...
![](C:/Users/k1894405/Documents/GitHub/GenomicPCA project/Genetic_networks_project/Path_diagram_Qtrait.jpg)


```{r qtrait, eval=T,warning=F,message=F,results='hide'}
#load dependencies
...
table$Qtrait <- signif(table$Qtrait, digits = 2)
```

```{r displayQtrait}
kable(table,row.names = FALSE,caption = "Qtrait statistics", digits = 24)
```
