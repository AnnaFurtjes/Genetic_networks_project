...
write.table(all_vars_cor, "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/pheno_preparation/assoc_volumes_covars_13042021.txt", sep = "\t", row.names = FALSE, col.names = TRUE)
...
#setwd(temporarywd)
assoc<-read.table("C:/Users/k1894405/OneDrive - King's College London/PhD/Projects/Genomic SEM project/data_my_own/Pheno_preparation/assoc_volumes_covars_13042021.txt",header=T)
...
Average correlations across all 83 volumes:
  ...
  - Mean *Site* correlation = `r round(mean(assoc$site_cor), digit=2)`; SD = `r round(sd(assoc$site_cor), digit=2)`
...
As stated in the pre-registration, we planned to include covariates with correlations above .10 as an arbitrary cut-off which is why we will include age and sex (but not site, time of year or the x, y, z coordinates), alongside genetic genotyping batch and 40 genetic principal components. 
...
As most covariates in this selection, apart from age and sex, are not associated with brain volumes more than .10, we only include age and sex and drop acquisition site, time of year and the x, y and z coordinates from the analyses. 
...
write.table(pheno_file, "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/pheno_preparation/target_phenotypes_83volumes_gwas.txt",sep = "\t", row.names = FALSE, col.names = TRUE, quote=F)

```
  
## Calculate phenotypic correlations

As this study aims to compare correlations of regional volumes between genetic and phenotypic measures, we calculate phenotypic correlations between all regional brain volumes considered in the genetic analyses.
...
save(list = c("cor_matrix","eigenvalues","eigenvectors","explained_variance","stand_loadings","age_cor"), file = "/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/anna/PhD/output/pheno_cor/pheno_decomposition.RData")
...
Note that the volumetric measures have been residualised for age and sex to match the phenotypes used in the GWAS. 