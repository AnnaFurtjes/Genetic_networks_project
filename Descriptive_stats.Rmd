...
names(descriptives_geno)[which(names(descriptives_geno) == "explained")] <- "geno_R2"
...
descriptives$pheno_R2 <- round(descriptives$pheno_R2, digits = 1)
descriptives$geno_R2 <- round(descriptives$geno_R2, digits = 1)
...
labels$label_position <- ifelse(labels$Modality == "pheno_R2", labels$label_x-13, labels$label_x+13)

R2_plot <- ggplot(data=descriptives, aes(y=networks),shape=3)+
                            xlim(0,100)+
                            geom_point(aes(x=R2, col = Modality), shape = 4, size=4, stroke = 2)+
...
plot_grid(loadings_pheno_plot,loadings_plot, R2_plot, nrow = 1,ncol = 3,labels=c("A","B","C"),width=1,height=1, label_size = 18,hjust = 0.07,scale = 0.85, rel_widths = c(1.5,1,1.5))
