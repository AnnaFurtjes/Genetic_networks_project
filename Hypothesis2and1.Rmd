...
## distribution of correlations for brain stem with rest of the brain 
#summary(gen_pheno[which(gen_pheno$region1 == "Brain_stem" & !is.na(gen_pheno$genetic_cor)| gen_pheno$region2 == "Brain_stem"),"genetic_cor"])
#brainstem_cor<-gen_pheno[which(gen_pheno$region1 == "Brain_stem" & !is.na(gen_pheno$genetic_cor)| gen_pheno$region2 == "Brain_stem"),"genetic_cor"]
# summary(brainstem_cor[2:length(brainstem_cor)])

## calculate mean absolute difference in phenotypic vs genetic correlation
#delta_cor<-abs(gen_pheno$phenotypic_cor-gen_pheno$genetic_cor)
#mean(delta_cor,na.rm=T)
# sd(delta_cor,na.rm=T)
# this is suggesting that phenotypic correlation is not a good predictor for genetic ones

## calculate explained variance betwen them
#summary(lm(gen_pheno$phenotypic_cor~gen_pheno$genetic_cor,na.action = "na.omit"))

# which correlations are negaitve?
#neg_gen<-gen_pheno[which(gen_pheno$genetic_cor<=0&!is.na(gen_pheno$genetic_cor)),]
# nrow(neg_gen)
# nrow(neg_gen)/sum(!is.na(gen_pheno$genetic_cor))

# test whether measurement error might be associated with the difference between pheno and geno correlations
#temporarywd<-paste0(workingd,"/data/heritability_volumes/")
#setwd(temporarywd)
#heritability<-fread("heritability_volumes_clean.txt",header=T,data.table = F)
# build avergage h2 for each combination of structure
# correlate that with delta_cor
```
...
### How many of the genetic correlations are negative?
```{r negative_gen_cor}
# negative genetic correlations
neg_cor<-sum(gen_pheno$genetic_cor<=0 & !is.na(gen_pheno$genetic_cor))
total_cor<-sum(!is.na(gen_pheno$genetic_cor))
percent_neg<-round((neg_cor/total_cor)*100,digits = 2)

# negative phenotypic correlations
neg_pheno<-sum(gen_pheno$phenotypic_cor<=0 & !is.na(gen_pheno$phenotypic_cor))
total_pheno<-sum(!is.na(gen_pheno$phenotypic_cor))
percent_neg_pheno<-neg_pheno/total_pheno
```

Genetic correlations are negative in `r percent_neg`% of the cases (negative correlations = `r neg_cor`, total correlations = `r total_cor`). In contrast, there are `r percent_neg_pheno`% negative phenotypic associations. 
...
temporarywd<-paste0(workingd,"/data/standardised_loadings/")
setwd(temporarywd)

gen_stand_loadings<-fread("stand_loadings_whole_brain.txt",data.table=F)
...
plot_loadings<-ggplot(stand_loadings, aes(x=gen_stand_loadings,y=pheno_stand_loadings))+#
  ...
  plot_loadings<-plot_loadings+xlab("Genetic PC loadings")+ylab("Phenotypic PC loadings")+
  theme(plot.title = element_text(size=20, hjust = 0.5), panel.background = element_blank(), axis.line = element_line(color="black"), axis.line.x = element_line(color="black"))+ theme_bw()+
    stat_cor(method="pearson",cor.coef.name="r",size=4.5,label.x.npc=0.65,label.y.npc = 0)+
  theme(legend.position = "none")+
    geom_abline(intercept=0,slope=(1),linetype="dashed",color="red",show.legend=T, size=1.5,alpha=0.6)+
    geom_vline(xintercept = 0,linetype="dashed",color="black",size=1.5,alpha=0.2)+
        theme(text = element_text(size=12,family="Arial Narrow"),
        axis.text.x = element_text(size=15,family="Arial Narrow"),#angle=45
        axis.text.y = element_text(size=15,family="Arial Narrow"),
...
temporarywd<-paste0(workingd,"/Scripts/Zhao_GWASs/3_perform_ldsc")
setwd(temporarywd)
...
plot_age_pheno<-ggplot(phenotest, aes(x=pheno_stand_loadings,y=age_cor))+#
  ...
  plot_age_pheno<-plot_age_pheno+xlab("Phenotypic PC loadings")+ylab("Age correlations")+
  theme(plot.title = element_text(size=20, hjust = 0.5), panel.background = element_blank(), axis.line = element_line(color="black"), axis.line.x = element_line(color="black"))+ theme_bw()+
    stat_cor(method="pearson",cor.coef.name="r",size=4.5,color="black",label.x.npc=0.65,label.y.npc = 0)+
  theme(legend.position = "none")+
        theme(text = element_text(size=12,family="Arial Narrow"),
        axis.text.x = element_text(size=15,family="Arial Narrow"),#angle=45
        axis.text.y = element_text(size=15,family="Arial Narrow"),
...
**This is the same result as reported in Madole et al.**
...
plot_age_geno<-plot_age_geno+xlab("Genetic PC loadings")+ylab("Age correlations")+
  theme(plot.title = element_text(size=20, hjust = 0.5), panel.background = element_blank(), axis.line = element_line(color="black"), axis.line.x = element_line(color="black"))+ theme_bw()+
    stat_cor(method="pearson",cor.coef.name="r",size=4.5,label.x.npc=0.65,label.y.npc = 0)+
      theme(text = element_text(size=12,family="Arial Narrow"),
        axis.text.x = element_text(size=15,family="Arial Narrow"),#angle=45
